...
def distance_to_set(word_hist, histograms):
    ...
    * dist: numpy.ndarray of shape (N)
    ...
    dist = np.empty(histograms.shape[0])

    for i in range (histograms.shape[0]):
        bhatt_coeff = np.sum(np.sqrt(np.multiply(word_hist.flatten(), histograms[i, :])))
        dist[i] = np.sqrt(1-bhatt_coeff)
        bhatt_coeff = 0 #Resetting Bhattacharyya Coefficient for next pass
    
    # mean_word = np.mean(word_hist)
    # mean_hist = np.mean(histograms)
    # var_word = np.var(word_hist)
    # var_hist = np.var(histograms)
    # print(mean_word)
    # print(var_hist)
    # dist = 1/4 * (mean_word * mean_hist)**2 / (var_word + var_hist) + 1 / 2 * np.log((var_hist + var_word) / (2 * var_word * var_hist))
    # print("Dist index:", np.argmin(dist))
    # print("histograms shape:", histograms.shape[0])
    return dist
