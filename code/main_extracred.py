...
def main():
    ...
    # Q2.5
    n_cpu = util.get_num_CPU()
    conf, accuracy = visual_recog_extracred.evaluate_recognition_system(opts, n_worker=n_cpu)

    print(conf)
    print(accuracy)
    np.savetxt(join(opts.out_dir, 'confmat_extracred.csv'), conf, fmt='%d', delimiter=',')
    np.savetxt(join(opts.out_dir, 'accuracy_extracred.txt'), [accuracy], fmt='%g')
